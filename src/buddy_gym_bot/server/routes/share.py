...
from ...db.models import User, WorkoutSession, SetRow
...
async def share_png(
    ...
    try:
        async with sessmaker() as s:
            ...
            if session_id == "last":
                ress = await s.execute(select(WorkoutSession).where(WorkoutSession.user_id == user.id).order_by(desc(WorkoutSession.started_at)).limit(1))
                ws = ress.scalar_one_or_none()
            else:
                ress = await s.execute(select(WorkoutSession).where(WorkoutSession.user_id == user.id, WorkoutSession.id == int(session_id)))
                ws = ress.scalar_one_or_none()
            if not ws:
                raise HTTPException(status_code=404, detail="No session")
            ressets = await s.execute(select(SetRow).where(SetRow.session_id == ws.id).order_by(SetRow.created_at.asc()))
    ...
    date_str = ws.started_at.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    ...
    draw.text((30, 80), f"User: @{user.handle or user.tg_id}    Date: {date_str}", font=font_small, fill=(200,200,200))
    draw.text((30, 120), f"Total Volume: {total_vol:.0f} kg", font=font_small, fill=(210,210,210))
    y = 170
    for r in sets[:8]:
        line = f"• {r.exercise}: {r.weight_kg:g} × {r.reps} " + (f"(RPE {r.rpe:g})" if r.rpe else "")
